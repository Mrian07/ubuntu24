...
echo "ahmadardhiansyah2020@gmail.com" > /etc/github/email
...
function add_subdomain() {
    echo -e "$COLOR1╭═══════════════════════════════════════════════════════════════════╮${NC}"
    echo -e "$COLOR1│ ${WH} MASUKKAN SUBDOMAIN YANG INGIN DITAMBAHKAN ${NC}${BIBlue}                       │${NC}"
    echo -e "$COLOR1│ ${WH} Pisahkan dengan koma jika lebih dari satu (misal: sg1,sg2,sg3) ${NC}${BIBlue}  │${NC}"
    echo -e "$COLOR1╰═══════════════════════════════════════════════════════════════════╯${NC}"
    echo " "

    # Minta pengguna untuk memasukkan subdomain yang dipisahkan dengan koma
    read -p "Masukkan subdomain: " subdomains

    # Pisahkan subdomain berdasarkan koma
    IFS=',' read -ra ADDR <<< "$subdomains"

    # Proses setiap subdomain
    for sub in "${ADDR[@]}"; do
        # Hapus spasi di sekitar subdomain
        sub=$(echo $sub | xargs)
        ...
        wget https://raw.githubusercontent.com/Mrian07/aingman-script/main/install/subdomain-vip.sh -O /tmp/subdomain-vip.sh
        chmod +x /tmp/subdomain-vip.sh

        # Jalankan skrip rmbl.sh dengan animasi
        /tmp/subdomain-vip.sh &
        show_spinner $!

        # Tampilkan progress bar
        echo "Menambahkan subdomain $sub..."
        progress_bar 5
...
progress_bar() {
    ...
    while [ $(echo "$progress < $duration" | bc) -eq 1 ]; do
        ...
        local filled=$(echo "($percent / 100) * $width" | bc -l | awk '{print int($1+0.5)}')
        ...
        progress=$(echo "$progress + $interval" | bc)
        sleep $interval
    done
    echo ""
}
...
clear;clear;clear
echo -e " $COLOR1╭════════════════════════════════════════════════════════╮${NC}"
echo -e " $COLOR1│ ${WH}               • SCRIPT BY PAKETVPN-VIP •              ${NC} $COLOR1│$NC"
echo -e " $COLOR1╰════════════════════════════════════════════════════════╯${NC}"
echo -e " $COLOR1      ══════════════════════════════════════════════${NC}"
